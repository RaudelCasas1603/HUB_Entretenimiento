...
function addToFavorites(id, tipo) {
    ...
    if (item) {
        ...
        localStorage.setItem(tipo, JSON.stringify(tipo === 'peliculas' ? peliculas : series));

        // Recargar las listas según corresponda
...
// Función para mostrar las peliculas o series favoritas
function showFavorites(tipo) {
    ...
    const favoritos = (tipo === 'peliculas' ? peliculas : series).filter(s => s.favorito);
    if (favoritos.length === 0) {
        console.log("No hay series o películas en favoritos.");
        contenedor.innerHTML = "<p>No hay películas o series en favoritos.</p>";
        return;
    }
